...
start(_Type, _Args) ->
    ...
    {ok, _} = cowboy:start_clear(fuck_rebar, [{port, 8080}], #{env => #{dispatch => Dispatch}}),
    ets:new(templates, [bag, named_table]),
    ets:insert(templates, {index, bbmustache:parse_file(wspad:asset("index.mustache"))}),
    logger:alert("fuck you, erlang"),
    erlang:spawn(
        fun() ->
...
loop() ->
    receive
        ...
        {updateAll, Name, Pad, SendingPid} ->
            ...
            lists:foreach(
                fun({_, Pid}) ->
                    logger:alert("Updating ~p / ~p from ~p", [Name, Pid, SendingPid])
                end,
                Pids
            ),
